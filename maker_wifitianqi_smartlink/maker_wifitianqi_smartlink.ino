...
String payload1 = "";
String webadd = "http://flash.weather.com.cn/wmaps/xml/zhengzhou.xml";    //接口地址 更改这个地址即可---"http://flash.weather.com.cn/wmaps/xml/zhengzhou.xml"
...
void open_picture()
{
  ...
  display.println("  SanSong");//输出字符并换行  
  display.setTextSize(1);             //设置字体大小  
  display.setTextColor(WHITE);        //设置字体颜色白色  
  display.setCursor(50,20);             //设置字体的起始位置 
  display.println("      2018");//输出字符并换行
  /*
  display.setTextSize(1);             //设置字体大小  
  display.setTextColor(WHITE);        //设置字体颜色白色  
  display.setCursor(50,50);             //设置字体的起始位置 
  display.println("pengwenzheng");//输出字符并换行
  */
  display.update();
...
void init1(){                //启动wifi连接
 ...
 while (WiFi.status() != WL_CONNECTED) {
  ...
  display.setCursor(0,16);
  display.print("SSID:");
  display.println(WiFi.psk().c_str());
  display.print("IP: ");
